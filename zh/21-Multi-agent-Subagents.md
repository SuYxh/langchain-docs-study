...
想象你是一家公司的 CEO（主智能体），你手下有几个部门经理（子智能体）：
- 市场部经理：专门负责市场调研
- 技术部经理：专门负责写代码
- 法务部经理：专门负责审合同

当用户提出一个需求时，你（CEO）不需要亲自去做所有事情，而是把任务分配给对应的部门经理去处理。经理完成后把结果汇报给你，你再整理好反馈给用户。

**核心要点：**
- 主智能体就像"大脑"，负责决策和分配任务
- 子智能体就像"手脚"，各司其职，专注于自己擅长的领域
- 用户只和主智能体对话，不直接和子智能体沟通

### 为什么要用子智能体？

1. **分工明确**：不同的子智能体专注不同领域，比如一个查天气、一个算数学、一个搜索信息
2. **上下文隔离**：每个子智能体都是"干净"的，不会被之前的对话干扰。就像每次开会都是新的白板
3. **并行处理**：主智能体可以同时派出多个子智能体干活，效率更高

### 两种调用方式：同步 vs 异步

**同步（等着干完）：**
- 就像你点外卖，站在门口等骑手送来才走
- 适合：必须要结果才能继续的任务，比如"查一下今天天气"

**异步（边等边干别的）：**
- 就像你下单了外卖，然后去做其他事情，外卖到了再去取
- 适合：耗时较长的任务，比如"帮我审查这份 100 页的合同"
- 需要三个工具配合：启动任务、查进度、取结果
...
```
主智能体有三个工具：
- call_research_agent（调研究员）
- call_writer_agent（调写手）
- call_reviewer_agent（调审核员）
```
优点：每个子智能体可以定制输入输出格式
缺点：子智能体多了工具也多，维护麻烦
...
- task(agent_name, description)
  - task("research", "调查 AI 行业现状")
  - task("writer", "写一篇关于 AI 的文章")
```
优点：添加新智能体很简单，不用改代码
缺点：自定义程度低一些
...
**输入端（给子智能体什么信息）：**
- 最简单：只传任务描述，比如"查北京天气"
- 复杂场景：可能需要传对话历史、用户偏好等额外信息

**输出端（子智能体返回什么）：**
- 默认：只返回最终答案文本
- 高级用法：可以用 `Command` 返回更多结构化数据

### 实际场景举例

**场景：用户问"帮我规划一次东京旅行"**

1. 主智能体接到任务，决定要调用多个子智能体
2. 调用"研究智能体"：查东京的景点、美食、交通
3. 调用"天气智能体"：查旅行期间的天气预报
4. 调用"写作智能体"：把收集到的信息整理成旅行攻略
5. 主智能体汇总所有结果，给用户一份完整的旅行规划

### 和路由器的区别

- **子智能体模式（Supervisor）**：主智能体是"全程陪同导游"，全程负责、多轮对话、记得上下文
- **路由器模式（Router）**：路由器是"前台接待"，只负责分配任务，不参与后续处理

### 一句话总结

子智能体模式就是"**CEO + 部门经理**"的分工协作架构：CEO（主智能体）负责接待客户、分配任务、汇总结果；部门经理（子智能体）各自专注于自己的领域，干完活向 CEO 汇报。这样既能处理复杂任务，又能保持各个子任务的独立性和专注度。
