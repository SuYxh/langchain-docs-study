...
| 章节 | 内容说明 | 涉及知识点 | 难度 |
|------|----------|-----------|------|
| **31. 智能客服系统** | 多场景客服机器人：FAQ 自动回答、专家路由转接、敏感操作拦截、退款审批 | RAG + Handoffs + HITL + 护栏 | ⭐⭐⭐⭐ |
| **32. AI 代码助手** | 多语言代码生成、代码解释重构、Bug 诊断修复、单元测试生成 | Skills + 工具调用 + 结构化输出 + MCP | ⭐⭐⭐⭐⭐ |
| **33. 数据分析助手** | 自然语言转 SQL、数据可视化、多数据源整合、分析报告生成 | Router + Subagents + 结构化输出 + Custom Workflow | ⭐⭐⭐⭐ |
| **34. AI 研究助手** | 多源文献检索、自动摘要提取、观点对比分析、研究报告撰写 | Agentic RAG + Subagents + MCP + 上下文管理 | ⭐⭐⭐⭐⭐ |
| **35. 自动化内容创作系统** | 多平台内容适配、素材收集、文章润色、SEO 优化、质量检查 | Subagents + 中间件 + 结构化输出 | ⭐⭐⭐ |
| **36. AI 旅游规划师** | 智能行程规划：目的地推荐、机票酒店查询、景点餐厅推荐、多日行程编排 | 多工具并行 + Router + 结构化输出 + HITL + 外部API | ⭐⭐⭐⭐ |
...
│   ├── ...
│   ├── 30-test.md
│   ├── 31-project-customer-service.md
│   ├── 32-project-code-assistant.md
│   ├── 33-project-data-analyst.md
│   ├── 34-project-research-assistant.md
│   ├── 35-project-content-creator.md
│   └── 36-project-travel-planner.md
