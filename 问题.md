...
22、使用 useStream 可以实现根据不同的消息类型，展示不同的组件吗？ 有些需要展示卡片，用户可以操作，有些展示文本即可



23、用户刷新页面，对话不会丢失 持久化是  useStream 做的吗？ 
...
24、 assistantId 参数有什么用？ 都写一样的会不会有问题？ 



25、useStream 对于后端有什么要求呢？ 



26、如何自定义请求头（比如加 API Key）, 使用 `FetchStreamTransport` 这个以后，其他的参数怎么传递呢，比如 assistantId、threadId 、 onCustomEvent 这些？还有 FetchStreamTransport 的 onRequest 这个方法有什么用呢？
...
27、模型的上下文和 Agent 的上下文是同一个吗？





28、interruptOn 配置，使用对象的时候是随意写的 key 吗？有什么依据？
...
29、多智能体和 skiils 分别适合于什么场景呢？他们之间有什么区别？ 



30、在多智能体协作中， **Subagents 模式**  和  **Router 模式**  有什么区别呢？各自的使用场景分别是什么呢？
...
32、langchain、langgraph、deepAgent 之间有什么区别呢？我再什么时候应该选择哪个呢？
...
99、给我一份 Agent 设计指南，告诉我什么模式适合什么场景？
